...
from dust3r.cloud_opt_flow.commons import (edge_str, ALL_DISTS, NoGradParamDict, get_imshapes, signed_expm1, signed_log1p,
                                      cosine_schedule, linear_schedule, cycled_linear_schedule, get_conf_trf)
import dust3r.cloud_opt_flow.init_im_poses as init_fun
...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        #print(set(self.edges), {(j, i) for i, j in self.edges})
    ...
    def save_conf_maps(self, path, start):
        ...
        for i, c in enumerate(conf):
            np.save(f'{path}/conf_{start+i}.npy', c.detach().cpu().numpy())
        return conf
    
    def save_init_conf_maps(self, path, start):
        conf = self.get_init_conf()
        for i, c in enumerate(conf):
            np.save(f'{path}/init_conf_{start+i}.npy', c.detach().cpu().numpy())
        return conf

    def save_rgb_imgs(self, path, start):
        imgs = self.imgs
        images = []
    ...
    def compute_global_alignment(self, init=None, init_priors=None, save_score_path=None, save_score_only=False, niter_PnP=10, **kw):
        if init is None:
            ...
            init_fun.init_minimum_spanning_tree(self, save_score_path=save_score_path, save_score_only=save_score_only, niter_PnP=niter_PnP, init_priors=init_priors)
