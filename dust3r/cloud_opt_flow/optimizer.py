...
import contextlib
import cv2
from dust3r.cloud_opt_flow.base_opt import BasePCOptimizer, edge_str
from dust3r.cloud_opt_flow.pair_viewer import PairViewer
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1', flow_loss_weight=0.0, 
                 ...
                 sintel_ckpt=False, use_self_mask=False, pxl_thre=50, sam2_mask_refine=False, motion_mask_thre=0.35, **kwargs):
    ...
    def get_flow(self, sintel_ckpt=False): #TODO: test with gt flow
        ...
        sintel_ckpt=False
        flow_net = load_RAFT() if sintel_ckpt else load_RAFT("third_party/RAFT/models/Tartan-C-T432x960-M.pth")
        flow_net = flow_net.to(device)
        flow_net.eval()
        if len(pair_imgs[0].shape)==3:
            pair_imgs = [pair_imgs[0][None], pair_imgs[1][None]]
        #print(self._ei)
        with torch.no_grad():
            chunk_size = 12
            flow_ij = []
    ...
    def get_motion_mask_from_pairs(self, view1, view2, pred1, pred2):
        ...
        # self.depth_maps_i[self.depth_maps_i>0.7] = 0.7
        # self.depth_maps_j[self.depth_maps_j>0.7] = 0.7
        #cv2.imwrite('1.png', self.depth_maps_i[0,0].cpu().numpy()*255)
        #print(self.depth_maps_i,self.depth_maps_i.shape)
        try:
          ...
          ego_flow_1_2, _ = self.depth_wrapper(self.R_i, self.T_i, self.R_j, self.T_j, 1 / (self.depth_maps_i + 1e-6), self.intrinsics_j, torch.linalg.pinv(self.intrinsics_i))
        try:
          ...
          ego_flow_2_1, _ = self.depth_wrapper(self.R_j, self.T_j, self.R_i, self.T_i, 1 / (self.depth_maps_j + 1e-6), self.intrinsics_i, torch.linalg.pinv(self.intrinsics_j))
