...
def train(args):
    ...
    if args.pretrained and not args.resume:
        ...
        # filtered_ckpt_state_dict = {k: v for k, v in ckpt_state_dict.items() if k in model_state_dict and v.shape == model_state_dict[k].shape}
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    #################################  only finetune the following module ###########################################
    # list_grad = ["downstream_head", "dec_blocks.8", "dec_blocks.9", "dec_blocks.10", "dec_blocks.11", "dec_norm",
    #                                 "dec_blocks2.8", "dec_blocks2.9", "dec_blocks2.10", "dec_blocks2.11"]
    list_grad = ["downstream_head", "dec_blocks", "dec_norm",
                                    "dec_blocks2",'dec_blocks_pc','patch_embed_point_cloud','zero_convs']
    print(model.named_parameters())
    for name, p in model.named_parameters():
        if not any([grad in name for grad in list_grad]):
