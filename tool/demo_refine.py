...
def get_reconstructed_scene(args, outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    imgs, raw_imgs = load_images(filelist, size=image_size, verbose=not silent, fps=fps, start=0, interval=interval, traj_format='custom', depth_prior_name=depth_prior_name, crop=False)
    if len(imgs) == 1:
    ...
    rescale_depth_maps(save_folder, scene.get_depthmaps(), raw_imgs)
...
def get_reconstructed_scene_hierachical(args, outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    rescale_depth_maps(save_folder, [torch.tensor(i).cuda() for i in all_depth_maps], raw_imgs)
    enlarge_seg_masks(save_folder, kernel_size=5 if use_gt_mask else 3) 
...
def rescale_depth_maps(path, depthmaps, imgs, start=0):
    model = PromptDA.from_pretrained("depth-anything/promptda_vitl").cuda().eval()
    images = []
    W, H = imgs[0].size
    ...
    for i in range(len(imgs)):
      ...
      prompt_depth_i = depthmaps[i][None, None]
      depth_map = model.predict(image_i, prompt_depth_i)
      depth_map = F.interpolate(depth_map, size=(H, W), mode='bilinear', align_corners=False)[0,0]
      ...
      np.save(f'{path}/frame_refine_{(start+i):04d}.npy', depth_map.detach().cpu().numpy())
